...
export default class ProductForm {
  ...
  getImages () {
    return this.productData['images'].map(({url, source}) => this.getImage(url, source));
  }
  ...
  uploadImage = async (event) => {
    ...
    input.onchange = () => {
      ...
      fetchJson('https://api.imgur.com/3/image/', {
        ...
        .then(response => {
          this.subElements['imageListContainer'].querySelector('ul').append(this.getImage(response.data.link, file.name));
        })
        .catch(err => console.log(err))
        .finally(() => {
          input.remove();
